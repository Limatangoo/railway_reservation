...
class trainRoutesAdd extends Screen
{
    /**
     * Query data.
     *
    ...
    public function add(train_routes $train_route, Request $request)
    {
        $mapped = Arr::map($request->get('train_routes'), function ($value, $key) {
            return ucfirst($value);
        });
        $findRouteId = DB::select('select id FROM train_routes WHERE id='.($request->get('train_routes'))['id'].'');
        
        
        if(count($findRouteId)>0){
            Alert::info('You already have train route with same route id');

            return redirect()->route('platform.trainroutes');
        }

       else if($train_route->fill($mapped)->save()){
            Alert::info('You have successfully added a new route.');
